...
interface BookmarkState {
  ...
  addBookmark: (param: { label: string; anchorTime: string; monthKey: string }) => WaterfallBookmark
   ...
   */
  updateAutoBookmark: (anchor: { label: string; monthKey: string; anchorTime: string }) => void
  /**
   * 确保自动书签存在：不存在（被删除重置或首次）时以传入锚点为起点生成。
   * 供页面在加载照片成功后调用，避免删除后因滚动立即重新生成。
   */
  ensureAutoBookmark: (anchor: { label: string; monthKey: string; anchorTime: string }) => void
...
function createState(storageKey: string): BookmarkState {
  ...
  function addBookmark(param: { label: string; anchorTime: string; monthKey: string }): WaterfallBookmark {
  ...
  function updateAutoBookmark(anchor: { label: string; monthKey: string; anchorTime: string }) {
  ...
  function ensureAutoBookmark(anchor: { label: string; monthKey: string; anchorTime: string }) {
    const existing = bookmarks.value.find((b) => b.id === AUTO_BOOKMARK_ID)
    if (existing) return
    ...
    persist()
    console.log(`[WaterfallBookmarks:${storageKey}] 加载后生成自动书签`, { anchorTime: anchor.anchorTime })
  }
  ...
  function removeAutoBookmark() {
    ...
    persist()
    console.log(`[WaterfallBookmarks:${storageKey}] 删除自动书签（重置，继续浏览后重新生成）`)
  }

  return { bookmarks, addBookmark, removeBookmark, updateAutoBookmark, ensureAutoBookmark, removeAutoBookmark }
}
