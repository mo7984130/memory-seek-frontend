...
import type { WaterfallGroup } from "@/components/visual/VirtualWaterfall.vue";

/**
 * 瀑布流页面 Composable
 *
 * 收敛三个页面（影像墙 / 我喜欢 / 收藏夹详情）共用的逻辑：
 * - 响应式列数与容器宽度
 * - 按月分组
 * - 触底加载（游标分页）
...
export interface UseWaterfallPageOptions {
  /** 持久化存储键名（visuals / likes / collection-<id>） */
  storageKey: string;
  /** 获取一页影像；cursor 为空表示从开头（或按 anchorTime） */
  fetch: (params: {
    cursor?: string;
    anchorTime?: string;
...
export function useWaterfallPage(options: UseWaterfallPageOptions) {
  ...
  const groups = computed<WaterfallGroup[]>(() => {
    if (!waterfall.allVisuals.value.length) return [];

    const map = new Map<string, Visual[]>();
    for (const visual of waterfall.allVisuals.value) {
      const key = visual.createdAt.substring(0, 7); // "2026-06"
      if (!map.has(key)) map.set(key, []);
      map.get(key)!.push(visual);
    }

    return Array.from(map.entries()).map(([key, visuals]) => ({
      key,
  ...
  // ======== 加载 ========
  /**
   * 请求一页影像：replace=true 时替换列表（恢复/时间线跳转），否则追加（触底加载）
   */
  async function requestPage(
    ...
    try {
      ...
      if (replace) {
        ...
        waterfall.appendVisuals(records, nextCursor ?? undefined, hasMore);
      }
      // 加载影像成功后确保"最远浏览位置"自动书签存在（被删除重置后在此重新生成）
      if (autoBookmarkEnabled) ensureAutoBookmark();
      return records;
    } catch (error) {
      console.error(
        `[WaterfallPage:${options.storageKey}] 获取影像失败:`,
        error,
      );
      return [];
    } finally {
  ...
  // ======== 浏览位置捕获 ========
  /** 当前视口顶部影像 ID（供页面生成位置书签锚点） */
  const topItemId = ref<string | number | null>(null);

  /**
   * 当前顶部影像的锚点信息；topItemId 尚未就绪时回退到列表第一张
   */
  function getTopAnchor(): {
    ...
    const visual =
      topItemId.value != null
        ? waterfall.allVisuals.value.find((p) => p.id === topItemId.value)
        : waterfall.allVisuals.value[0];
    if (!visual?.createdAt) return null;
    return {
      label: AUTO_BOOKMARK_LABEL,
      monthKey: visual.createdAt.substring(0, 7),
  ...
  /**
   * 确保自动书签存在（加载影像成功后调用）：
   * 被删除重置后不会因滚动立即重新生成，而是等下次加载影像时生成
   */
  ...
  async function restoreToLastPosition(): Promise<void> {
    ...
    if (saved.cursor != null) {
      const pageStartId = waterfall.getPageStartVisualId(saved.cursor);
      if (pageStartId) {
        await nextTick();
    ...
    if (
      ...
      const firstId = waterfall.getPageStartVisualId(null);
      if (firstId) {
        await nextTick();
